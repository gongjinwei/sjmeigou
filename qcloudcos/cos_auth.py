...
class Auth(object):
    def __init__(self, appid, SecretID, SecretKey, bucket, region, method, objectName, head="", parameters="", sign_time="", key_time="",expire=10000):
        """
        ...
        if sign_time == "":
            self.q_sign_time = int(time.time())-60
        else:
            self.q_sign_time = int(sign_time)

        if key_time == "":
            self.q_key_time = int(self.q_sign_time) + expire
        else:
